...
class RateLimiter:
    ...
    def process_call(self, chat_id: Optional[int], user_id: Optional[int]) -> bool:
        ...
        if chat_id_is_not_none and self._limit.check_chat_limit():
            chat_calls = self._call_tracker.get_chat_calls(chat_id)

            if chat_calls < self._limit.chat_limit_calls:
                self._processed_chat_ids.discard(chat_id)
            elif chat_calls == self._limit.chat_limit_calls:
                if chat_id not in self._processed_chat_ids:
                    self._processed_chat_ids.add(chat_id)
                    self._limit.process_chat_over_limit(chat_id)

                is_limited = True

        if user_id_is_not_none and self._limit.check_user_limit():
            user_calls = self._call_tracker.get_user_calls(user_id)

            if user_calls < self._limit.user_limit_calls:
                self._processed_user_ids.discard(user_id)
            elif user_calls == self._limit.user_limit_calls:
                if user_id not in self._processed_user_ids:
                    self._processed_user_ids.add(user_id)
                    self._limit.process_user_over_limit(user_id)

                is_limited = True

        if chat_id_is_not_none and user_id_is_not_none and self._limit.check_chat_user_limit():
            chat_user_calls = self._call_tracker.get_chat_user_calls(chat_id, user_id)
            chat_user_ids = (chat_id, user_id)

            if chat_user_calls < self._limit.chat_user_limit_calls:
                self._process_chat_user_ids.discard(chat_user_ids)
            elif chat_user_calls == self._limit.chat_user_limit_calls:
                if (chat_id, user_id) not in self._process_chat_user_ids:
                    self._process_chat_user_ids.add((chat_id, user_id))
                    self._limit.process_chat_user_over_limit(chat_id, user_id)
