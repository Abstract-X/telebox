...
from tests.dispatcher.dispatcher.helpers import EditedMessageHandler, EditedMessageFilter
...
TEST_FILTER_DATA = (
    ...
    (
        ...
        (EditedMessageFilter(),),
        (~EditedMessageFilter(),),
        (EditedMessageFilter() & EditedMessageFilter(),),
        (EditedMessageFilter() | EditedMessageFilter(),)
    )
)
...
def test(
    ...
    assert dispatcher.check_edited_message_handler(handler, filter_, rate_limit) is False

    dispatcher.add_edited_message_handler(handler, filter_, rate_limit)

    assert dispatcher.check_edited_message_handler(handler, filter_, rate_limit) is True
...
def test_default_rate_limit(token: str) -> None:
    ...
    handler = EditedMessageHandler()
    filter_ = EditedMessageFilter()

    assert dispatcher.check_edited_message_handler(handler, filter_, rate_limit) is False

    dispatcher.add_edited_message_handler(handler, filter_)
