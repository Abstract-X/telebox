...
from tests.dispatcher.dispatcher.helpers import MessageHandler, MessageFilter
...
def test(
    ...
    assert dispatcher.check_message_handler(handler, filter_, rate_limit) is False

    dispatcher.add_message_handler(handler, filter_, rate_limit)

    assert dispatcher.check_message_handler(handler, filter_, rate_limit) is True
