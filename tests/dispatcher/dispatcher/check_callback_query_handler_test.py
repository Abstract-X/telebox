...
from tests.dispatcher.dispatcher.helpers import CallbackQueryHandler, CallbackQueryFilter
...
def test(
    ...
    assert dispatcher.check_callback_query_handler(handler, filter_, rate_limit) is False

    dispatcher.add_callback_query_handler(handler, filter_, rate_limit)

    assert dispatcher.check_callback_query_handler(handler, filter_, rate_limit) is True
