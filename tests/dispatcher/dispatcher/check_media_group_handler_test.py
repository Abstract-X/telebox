...
from tests.dispatcher.dispatcher.helpers import MediaGroupHandler, MediaGroupFilter
...
def test(
    ...
    assert dispatcher.check_media_group_handler(handler, filter_, rate_limit) is False

    dispatcher.add_media_group_handler(handler, filter_, rate_limit)

    assert dispatcher.check_media_group_handler(handler, filter_, rate_limit) is True
